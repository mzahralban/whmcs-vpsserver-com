...
function initDataSelectors() {
    if (!(typeof mgGlobalAppConfig !== "undefined" && (typeof mgGlobalAppConfig['autoInitTooltips'] !== "undefined") && mgGlobalAppConfig['autoInitTooltips'] == false)) { 
...
var forms = [].slice.call(document.querySelectorAll('.form-control'));

forms.forEach(function (item) {
    var $item = $(item);

    item.addEventListener('focus', function () {
        $item.closest('.input-group').addClass('is-focus');
    });
    item.addEventListener('blur', function () {
        $item.closest('.input-group').removeClass('is-focus');
    });
});
...
var SELECTORS = {
    select: ((typeof mgGlobalAppConfig !== "undefined" && (typeof mgGlobalAppConfig['appScope'] !== "undefined")) ? (mgGlobalAppConfig['appScope'] + ' ' + 'select') : 'select'),
...
function initDataSelectors() {
    if (!(typeof mgGlobalAppConfig !== "undefined" && (typeof mgGlobalAppConfig['autoInitSelects'] !== "undefined") && mgGlobalAppConfig['autoInitSelects'] == false)) { 
...
var alert = {
    ...
    getTemplate: function getTemplate() {
        ...
        // main alert template   
        var $primaryTemplate = '<div class="lu-alert lu-alert--' + this.$alertStatus + ' lu-alert--outline lu-alert--float lu-alert--border-left has-icon">' + '<div class="lu-alert__body">' + $titleTemplate + '<p>' + this.$alertBody + '</p>' + '</div>' + this.$actions + '</div>';
...
var layersNav = {
    ...
    cacheDom: function cacheDom() {
        this.$nav = $('.nav, .lu-nav');
        this.$navigationDroDownLi = this.$nav.find('.nav__item.has-dropdown, .lu-nav__item.has-dropdown');
    },
    bindEvents: function bindEvents() {
        var that = this;
            ...
            unmatch: function unmatch() {
                $('.has-dropdown > a').on('click', function (event) {
                    if ($(this).parents('.navigation').length == 0 &&
                        (isMobile == true || $(this).parents('.lu-navbar--thin').length == 0)) {
        ...
        $(window).on('click', function (event) {
            ...
            $('.lu-nav .has-dropdown').removeClass('is-open');
        }.bind(this));
    },
...
var Tab = function () {
  ...
  var Selector = {
    DROPDOWN: '.dropdown',
    NAV_LIST_GROUP: '.nav, .list-group, .lu-nav, .lu-list-group',
    ACTIVE: '.is-active',
    ACTIVE_UL: '> .is-active',
...
var _navs = __webpack_require__(19);

var _navs2 = _interopRequireDefault(_navs);

var _switcher = __webpack_require__(20);
...
var _CheckAll = __webpack_require__(12);
...
var _tooltips = __webpack_require__(11);
...
var _contentSlider = __webpack_require__(9);
...
var _inlineEditor = __webpack_require__(16);

var _inlineEditor2 = _interopRequireDefault(_inlineEditor);

__webpack_require__(8);

var _sticky = __webpack_require__(25);

var _sticky2 = _interopRequireDefault(_sticky);

var _toggler = __webpack_require__(26);
...
(function (root, factory) {
  ...
  */
	...
	var Selectize = function Selectize($input, settings) {
		...
		self.settings.mode = self.settings.mode || (self.settings.maxItems === 1 ? 'single' : 'lu-multi');
		if (typeof self.settings.hideSelected !== 'boolean') {
			self.settings.hideSelected = self.settings.mode === 'lu-multi';
		}
		...
		setupTemplates: function setupTemplates() {
			...
			var templates = {
				...
				'item': function item(data, escape) {
					return '<div class="lu-item">' + escape(data[field_label]) + '</div>';
				},
   ...
   */
		onKeyPress: function onKeyPress(e) {
			...
			if (this.settings.create && this.settings.mode === 'lu-multi' && character === this.settings.delimiter) {
				this.createItem();
				e.preventDefault();
				return false;
			}
   ...
   */
		onItemSelect: function onItemSelect(e) {
			...
			if (self.settings.mode === 'lu-multi') {
				e.preventDefault();
   ...
   */
		addItem: function addItem(value, silent) {
			...
			debounce_events(this, events, function () {
				...
				if (inputMode === 'lu-multi' && self.isFull()) return;
   ...
   */
		open: function open() {
			...
			if (self.isLocked || self.isOpen || self.settings.mode === 'lu-multi' && self.isFull()) return;
			self.focus();
			self.isOpen = true;
			self.refreshState();
  ...
  */
   ...
   */
	...
	Selectize.define('drag_drop', function (options) {
		...
		if (this.settings.mode !== 'lu-multi') return;
		var self = this;
